...
기본 경로가 아닌 곳에 설치했다면 `--prefix DIR`을 지정합니다. 설치 시 확인 schedule만 생성하며 update를 즉시 적용하지는 않습니다. 설치 중에는 `install.sh --disable-auto-update`, 설치 후에는 `traceknot-update disable`로 opt-out할 수 있습니다. 전체 정책, 복구 동작, release contract, 검증 근거는 [`docs/automatic-updates.md`](docs/automatic-updates.md)에 정리되어 있습니다.
