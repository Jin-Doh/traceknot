...
观看 Codex（`gpt-5.6-luna`）将 Traceknot 应用于真实变更：自动发现 Skill，由 Verify CLI 收集显式命令证据，并从隔离运行中发布会话 QA 看板。

![运行 gpt-5.6-luna 的 Codex 应用 Traceknot 验证并发布 会话 QA 看板](assets/readme/traceknot-codex-board.gif)
