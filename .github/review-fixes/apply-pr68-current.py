...
def subone(text: str, pattern: str, repl: str, label: str, flags: int = 0) -> str:
...
def patch_deadline(text: str, label: str) -> str:
    pattern = r'''run_with_deadline\(\) \{\n.*?\n\}\n\ndurable_sync\(\) \{'''
    repl = '''run_with_deadline() {
...
durable_sync() {'''
    return subone(text, pattern, repl, f"{label}: deadline runner", re.S)


def patch_recovery(text: str, label: str) -> str:
    text = one(text, 'LOCK_OWNER_ID=\n', 'LOCK_OWNER_ID=\nRECOVERY_CLAIM=\n', f'{label}: recovery claim var')
    text = one(
        text,
    ...
    if [ -L "$RECOVERY_LOCK" ]; then
        ...
        case "$recovery_pid" in
            ''|*[!0-9]*) fail "invalid stale-lock recovery PID: $RECOVERY_LOCK" ;;
        esac
        ...
        command -v flock >/dev/null 2>&1 ||
            fail "unsafe stale-lock recovery path: $RECOVERY_LOCK"
        [ -f "$RECOVERY_LOCK" ] || fail "unsafe stale-lock recovery path: $RECOVERY_LOCK"
    fi
    if command -v flock >/dev/null 2>&1; then
        ...
        mkdir "$RECOVERY_LOCK" 2>/dev/null ||
            fail "stale-lock recovery is already in progress: $RECOVERY_LOCK"
        recovery_owner_file=$RECOVERY_LOCK/owner
        printf '%s\n%s\n' "$$" "$LOCK_OWNER_ID" > "$recovery_owner_file"
        RECOVERY_LOCK_HELD=3
    fi
}'''
    new = '''acquire_recovery_guard() {
    if [ -L "$RECOVERY_LOCK" ]; then
        fail "unsafe stale-lock recovery path: $RECOVERY_LOCK"
    elif [ -d "$RECOVERY_LOCK" ]; then
        recovery_owner_file=$RECOVERY_LOCK/owner
    ...
    return one(text, old, new, f'{label}: recovery guard')


def patch_skills(path: Path) -> None:
    text = path.read_text()
    text = patch_deadline(text, str(path))
    text = patch_recovery(text, str(path))
...
''',
        ...
        command -v diff >/dev/null 2>&1 || fail 'diff is required to reconcile a pending update'
''',
        f'{path}: pending early self-check removal',
    )
    text = one(
        text,
        '''        if ! diff -r "$PENDING_PAYLOAD_DIR" "$REGISTRATION" >/dev/null 2>&1; then
...
run_with_deadline "$PREFLIGHT_SKILL/bin/traceknot" self-check >/dev/null 2>&1 ||
    fail 'preflight runtime self-check failed'
''',
        f'{path}: preflight verify before execute',
    )
    text = one(
        text,
...
run_with_deadline "$REGISTRATION/bin/traceknot" self-check >/dev/null 2>&1 ||
    fail 'updated runtime self-check failed'
''',
        f'{path}: installed verify before execute',
    )
    text = one(
        text,
...
''',
        f'{path}: matching manual adoption',
    )
    text = one(
        text,
...
''',
        f'{path}: pre-mutation revalidation',
    )
    text = one(
        text,
...
CURRENT_LOCK_REF=$(jq -r '.skills.traceknot.ref // empty' "$LOCK_FILE")
write_pending_state''',
        f'{path}: final pre-pending revalidation',
    )
    text = one(
        text,
...
def patch_legacy(path: Path) -> None:
    text = path.read_text()
    text = patch_recovery(text, str(path))
    path.write_text(text)
...
''', 'tests: preflight mutation fixtures')
...
''', 'tests: apply poison fixture')
    text = one(text, '''[ "$((END_TIMEOUT - START_TIMEOUT))" -lt 6 ]
test ! -e "$TIMEOUT_STATE/update.lock"

# Global registration:''', '''[ "$((END_TIMEOUT - START_TIMEOUT))" -lt 6 ]
test ! -e "$TIMEOUT_STATE/update.lock"
...
# Global registration:''', 'tests: kill escalation')
    anchor = '''# Global registration: opt-in scheduling, strict seven-day boundary, exact-commit apply.
...
GLOBAL_SKILL=$HOME/.agents/skills/traceknot
'''
    text = one(text, anchor, block, 'tests: current review regressions')
...
def patch_docs(path: Path) -> None:
    text = path.read_text()
    text = one(text, '''The update lock records both the updater PID and its process-start identity. A live PID with a different identity is treated as a stale lock; a legacy live lock without an identity is rejected rather than guessed.
''', '''The update lock records both the updater PID and its process-start identity. A live PID with a different identity is treated as a stale lock; a legacy live lock without an identity is rejected rather than guessed. When `flock` is unavailable, recovery ownership is published atomically through a fully written hard-link claim, and an empty directory left by the pre-release fallback implementation is safely reclaimed.
''', 'docs: fallback guard')
    text = one(text, '''The durable pending record retains both the verified candidate payload and the previous registration payload until reconciliation. A payload mismatch is a hard failure and never promotes `active.json`.
''', '''The durable pending record retains both the verified candidate payload and the previous registration payload until reconciliation. A payload mismatch is a hard failure and never promotes `active.json`. Candidate and reconciled payloads are compared byte-for-byte before their runtime is executed. Immediately before the canonical mutation, the updater revalidates the complete Skills lock entry and the live registration against snapshots captured before remote preparation; concurrent manual updates fail closed.
''', 'docs: revalidation and execution order')
    text = one(text, '''Every `--auto` invocation receives a 900-second absolute operation deadline by default. Release requests use the remaining budget for connection and transfer timeouts, and provenance, Skills CLI, archive, runtime self-check, and durability subprocesses are terminated when the same deadline expires. `TRACEKNOT_UPDATE_OPERATION_TIMEOUT` may set another positive deadline for controlled environments. `TRACEKNOT_UPDATE_DEADLINE_EPOCH` may provide an earlier absolute epoch deadline; the updater always honors the earlier bound.
''', '''Every `--auto` invocation receives a 900-second absolute operation deadline by default. Release requests use the remaining budget for connection and transfer timeouts, and provenance, Skills CLI, archive, runtime self-check, and durability subprocesses are terminated when the same deadline expires. The timeout watchdog completes TERM-to-KILL escalation before returning. `TRACEKNOT_UPDATE_OPERATION_TIMEOUT` may set another positive deadline for controlled environments. `TRACEKNOT_UPDATE_DEADLINE_EPOCH` may provide an earlier absolute epoch deadline; the updater always honors the earlier bound.
''', 'docs: kill escalation')
    path.write_text(text)


for p in [Path('bin/traceknot-skills-update'), Path('skill/bin/traceknot-skills-update')]:
    patch_skills(p)
patch_legacy(Path('bin/traceknot-update'))
patch_tests(Path('tests/skills-updater-smoke.sh'))
patch_docs(Path('docs/automatic-updates.md'))
if Path('bin/traceknot-skills-update').read_bytes() != Path('skill/bin/traceknot-skills-update').read_bytes():
    raise SystemExit('Skills updater mirror drift')
print('applied current PR68 review fixes')
