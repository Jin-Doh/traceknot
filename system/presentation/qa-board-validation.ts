...
export function parseQaBoardManifest(value: unknown): QaBoardManifest {
  ...
  if (Number(counts.mandatory) !== ["passed", "failed", "blocked", "incomplete"].reduce((total, key) => total + Number(counts[key]), 0)) throw new Error("Board manifest counts are inconsistent");
