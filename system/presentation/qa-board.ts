...
export function renderQaBoardHtml(view: QaBoardView, locale: QaBoardLocale = "en"): string {
  ...
  const coverageRows = Object.entries(view.coverage).map(([name, value]) => {
    const percent = value.total === 0 ? 0 : Math.round(value.covered / value.total * 100);
    return `<tr><th scope="row"><span>${escapeHtml(coverageLabel(name, locale))}</span><progress max="100" value="${percent}" aria-label="${escapeHtml(coverageLabel(name, locale))}: ${percent}%"></progress></th><td data-label="${copy.covered}"><strong>${value.covered} / ${value.total}</strong><span class="percentage">${percent}%</span></td><td data-label="${copy.uncoveredIds}">${escapeHtml(value.uncoveredIds.join(", ") || "—")}</td></tr>`;
  }).join("");
