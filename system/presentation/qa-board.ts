...
export function renderQaBoardHtml(view: QaBoardView): string {
  ...
  const coverageRows = Object.entries(view.coverage).map(([name, value]) => `<tr><th>${escapeHtml(name)}</th><td>${value.covered} / ${value.total}</td><td>${escapeHtml(value.uncoveredIds.join(", ") || "—")}</td></tr>`).join("");
...
<style>
:root{color-scheme:light dark;font-family:ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:#f4f5f7;color:#15171a}*{box-sizing:border-box}body{margin:0}.page{width:min(1120px,100%);margin:auto;padding:32px 20px 56px}.hero,.panel,.finding{background:#fff;border:1px solid #d9dde3;border-radius:14px;box-shadow:0 1px 2px #0000000d}.hero{padding:28px}.eyebrow,.meta,.muted{color:#626a76}.eyebrow{font-size:.78rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase}.verdict{display:inline-block;margin:12px 0 6px;padding:7px 12px;border-radius:999px;background:#1f2937;color:#fff;font-weight:750}.counts{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:10px;margin-top:20px}.count{padding:12px;border-radius:10px;background:#f4f5f7}.count strong{display:block;font-size:1.35rem}.panel{margin-top:18px;padding:22px}.finding{padding:18px;margin-top:12px;box-shadow:none;border-left-width:5px}.finding header{display:flex;align-items:center;gap:10px;flex-wrap:wrap}.finding h3{margin:0;font-size:1rem;overflow-wrap:anywhere}.finding h4{margin-bottom:4px}.status{font-size:.76rem;font-weight:750;text-transform:uppercase}.status-fail{border-left-color:#b42318}.status-blocked{border-left-color:#b54708}.status-incomplete{border-left-color:#667085}.status-pass{border-left-color:#067647}.rejected{color:#b42318}.previews{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px}.previews figure{margin:0}.previews img{display:block;width:100%;height:auto;border:1px solid #d9dde3;border-radius:8px}.previews figcaption{margin-top:5px;color:#626a76;font-size:.8rem}table{width:100%;border-collapse:collapse}th,td{padding:9px;text-align:left;border-bottom:1px solid #e4e7ec;overflow-wrap:anywhere}details{margin-top:18px}.notice{border:1px solid #f0b429;background:#fff8e1;border-radius:10px;padding:14px;color:#533f03}@media(max-width:700px){.page{padding:18px 12px 40px}.hero,.panel{padding:18px}.counts{grid-template-columns:repeat(2,minmax(0,1fr))}table{font-size:.84rem}}@media(prefers-color-scheme:dark){:root{background:#111418;color:#f4f5f7}.hero,.panel,.finding{background:#191d23;border-color:#303640}.eyebrow,.meta,.muted,figcaption{color:#aab2bf}.count{background:#242a32}th,td{border-color:#303640}.notice{background:#332b12;color:#ffe7a3;border-color:#806b23}}
</style>
</head>
...
<section class="panel"><h2>Coverage</h2><table><thead><tr><th>Area</th><th>Covered</th><th>Uncovered IDs</th></tr></thead><tbody>${coverageRows}</tbody></table></section>
