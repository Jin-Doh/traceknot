...
<section class="assurance-strip" aria-label="${copy.assurance}"><strong>${copy.assurance}</strong><span>${copy.assuranceContext}: ${escapeHtml(view.assurance.context)}</span><span>${copy.requiredIndependence}: ${escapeHtml(view.assurance.requiredIndependence)}</span><span>${copy.releaseStatus}: ${escapeHtml(view.assurance.releaseStatus)}</span></section>
