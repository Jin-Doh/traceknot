...
export async function executeObligations(input: ExecuteObligationsInput): Promise<ExecutionDocument> {
  ...
  for (const obligation of [...input.plan.obligations].sort((a, b) => a.id.localeCompare(b.id))) {
    ...
    if (verdict === "PASS" || verdict === "FAIL") {
      ...
      for (const artifact of inputs) { const stored = await storeArtifact(input.dependencies.artifactStore, artifact, request); artifacts.push(stored.type === "verification-result" ? stored : { ...stored, type: "verification-result" }); await record(input.dependencies.usageRecorder, { runId: input.runId, obligationId: obligation.id, event: "artifact" }); }
    }
...
export async function evaluateEvidence(input: EvaluateEvidenceInput): Promise<EvidenceDocument> {
  ...
  const claimsById = new Map(input.execution.claims.map(claim => [claim.claimId, claim])); const accepted = new Set(evaluations.filter(item => item.status === "ACCEPTED").map(item => claimsById.get(item.claimId)?.obligationId).filter((id): id is string => Boolean(id))); const conditionById = new Map(input.plan.conditions.map(item => [item.id, item])); const conditions = uniq(input.plan.conditions.map(item => item.id)); const coveredConditions = uniq(input.plan.obligations.filter(item => accepted.has(item.id)).flatMap(item => item.conditionIds));
...
export async function runVerification(input: RunVerificationInput): Promise<RunVerificationResult> {
  ...
  validRequest(request!); const req = request!; const documents: VerificationRunDocuments = { request: persisted ?? req };
