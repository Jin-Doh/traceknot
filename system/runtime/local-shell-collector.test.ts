...
describe("LocalShellCollector", () => {
  ...
  test("rejects declared artifact mismatches and invalid limits", async () => {
    ...
    try {
      ...
      const partial = [{ type: "structured", digest: digest(Buffer.from("complete")), path: "result.bin" }];
      const partialFailure = await collectorFor(root, store).collect({ ...requestFor(root), argv: ["-e", "process.exit(8)"], declaredArtifacts: partial, bestEffortDeclaredArtifactsOnFailure: true });
      expect({ status: partialFailure.execution.exitStatus, exitCode: partialFailure.execution.exitCode, partialPublished: partialFailure.artifacts.some(artifact => artifact.digest === partial[0]!.digest) }).toEqual({ status: "failed", exitCode: 8, partialPublished: false });
