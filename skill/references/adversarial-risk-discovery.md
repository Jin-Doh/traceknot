...
The JSON parser boundary remains authoritative for raw duplicate object keys: a parser that overwrites a duplicate key cannot recover both profile values. Migration from the old array/material representation is `materialTrigger || any old material`; the canonical object form keeps only the root `materialTrigger`.
