...
This invocation is maintenance advice outside the governed verification run. By default the helper reuses the sibling updater's local `lastCheckLocal` completion timestamp and skips another eligibility check for 24 hours in the same local clock domain; the updater's trusted `lastCheck` remains the eligibility source. The advisory invokes a read-only maintenance mode: status does not reclaim or reconcile recovery state, and check exits silently when pending recovery is present; on a clean check it sets the maintenance-only skip-sync flag so a slow filesystem flush cannot block completion. It writes a recommendation only to standard error when a newer release satisfies the seven-day policy. A missing helper, recent check, timeout, dependency failure, network failure, or malformed result is silent and MUST NOT change evidence, coverage, Board status, harness completion, or the QA verdict.
