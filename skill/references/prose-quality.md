...
Use the prose-quality obligation only for repository paths declared as Korean or English publication prose. It assesses observable style and preservation properties; it does not infer whether a person or an AI authored the text.
...
- `PQ-RISK-004` (`R1`): rules for one language are applied to another language.
...
| `PQ-COND-002` locale routing | Korean and English rules apply only to matching prose | `PQ-OBL-002` test result | separate-verification-context |
