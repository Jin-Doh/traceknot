...
Default to `evidence-only`. A runtime handshake MAY advertise command execution, browser execution, artifact capture, snapshot binding, independent evidence, evidence persistence, exception approval, isolated read-only review, enforced structured output, or Skill-origin egress denial enforcement. The host name or model name never implies a capability or producer independence.
