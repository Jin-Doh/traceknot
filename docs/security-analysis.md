...
| `SEC-P0-006` | A Skill-origin outbound request must be denied before transmission and resolve the affected mandatory obligation to `FAIL`. | `system/runtime/skill-egress-policy.ts`; host-issued `TrustedExecutionScope`, `EgressAuthorization`, and `executeGovernedEgress` must precede a `GovernedTransport`. | Blocking |
