...
| `SEC-P0-006` | A Skill-origin outbound request must be denied before transmission and resolve the affected mandatory obligation to `FAIL`. | `system/runtime/skill-egress-policy.ts`; host must call `executeSkillEgress` before its transmitter. | Blocking |
