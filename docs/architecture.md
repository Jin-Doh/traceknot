...
The versioned `quality-capability/v3` contract separates the legacy execution capabilities from an `enforcementProfile` containing `originAttribution`, `toolMediation`, `processIsolation`, and `auditDurability`. The hardened egress claim is derived only when the profile has non-null origin attribution, all-tool mediation, network-denied process isolation, and pre-transmit-durable evidence. A v2 record is never upgraded into that claim; it remains false.
