...
The `zh-Hans` path delegates Markdown-aware typography, punctuation, and Chinese/Western spacing diagnostics to the maintained `zhlint` library. Traceknot calls its Node API with a fixed repository preset, validates that every result remains bound to the original source and valid offsets, and reports diagnostics without applying `zhlint` autofixes. The adapter does not add an open-ended Chinese vocabulary or grammar layer.
