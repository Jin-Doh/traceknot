...
Inline-link validation follows visible Markdown structure: it requires a balanced, unescaped label opener; tracks block transitions and blockquote-container depth before classifying indentation or nested fences; ignores valid single- or multiline code spans and HTML comments; preserves balanced destination parentheses; and classifies URI schemes plus scheme-relative network paths before local-path resolution. Single- or multiline reference definitions and URL-bearing attributes on actual visible HTML tags, including complete `srcset` candidate lists with data URLs, use the same repository-boundary checks. Language navigation is checked against those visible parsed targets rather than raw source substrings. Shared command blocks likewise require a valid opener info string and a matching line-anchored closing fence without a non-whitespace suffix.
