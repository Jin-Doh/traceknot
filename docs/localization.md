...
Rewrite preservation tokenizes Simplified Chinese before binding normative clauses and quantities. It coalesces adjacent or unit-attached written-numeral segments, keeps signed Arabic and written values attached to grammatical counters or measurement terminals, and contextually excludes an explicit family of ambiguous lexical number-counter compounds. Original Markdown line boundaries, balanced link destinations, container markers, inline subject formatting, and positionally trailing modifiers before `有` are normalized before deriving quantity subjects. Concise obligation, permission, and prohibition modals bind across single-character modifiers to bounded Han clauses without a finite action-verb list; productive `可…性` adjectives are recognized only when `性` immediately follows the first adjective segment, so permissions whose later object contains a `性` noun remain protected. These are deterministic integrity checks rather than a substitute for human translation review.
