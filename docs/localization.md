...
Inline-link validation follows visible Markdown structure: it requires a balanced, unescaped label opener; tracks block transitions plus blockquote and list-container boundaries before classifying indentation or nested fences; recognizes ATX and Setext headings as completed non-paragraph blocks; ignores valid single- or multiline code spans and HTML comments; preserves balanced destination parentheses; and classifies URI schemes plus scheme-relative network paths before local-path resolution. Fence masking and shared command extraction use the same opener-info validation, including the backtick-fence restriction, and require a matching line-anchored closing fence without a non-whitespace suffix. Single- or multiline reference definitions and URL-bearing attributes on actual visible HTML tags, including complete `srcset` candidate lists with data URLs, use the same repository-boundary checks. Language navigation is checked against those visible parsed targets rather than raw source substrings.
