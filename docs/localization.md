...
Rewrite preservation tokenizes Simplified Chinese before binding normative clauses and quantities. It coalesces adjacent or unit-attached written-numeral segments, keeps signed Arabic and written values attached to grammatical counters or measurement terminals, and contextually excludes ambiguous lexical number-counter compounds. Original Markdown line boundaries, balanced link destinations, container markers, inline subject formatting, and trailing clause modifiers are normalized before deriving quantity subjects. Concise obligation, permission, and prohibition modals bind across single-character modifiers to bounded Han clauses without a finite action-verb list, while fixed lexical compounds and productive `可…性` adjectives remain ordinary prose. These are deterministic integrity checks rather than a substitute for human translation review.
