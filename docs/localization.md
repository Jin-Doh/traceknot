...
Inline-link validation follows visible Markdown structure: it requires a balanced, unescaped label opener; tracks whether indentation can actually start code; ignores top-level or container-nested fences, single- or multiline code spans, and HTML comments; preserves balanced destination parentheses; and classifies any syntactically valid URI scheme before local-path resolution. Single- or multiline reference definitions and URL-bearing attributes on actual visible HTML tags, including complete `srcset` candidate lists with data URLs, use the same repository-boundary checks. Language navigation is checked against those visible parsed targets rather than raw source substrings. Shared command blocks likewise require a matching line-anchored closing fence without a non-whitespace suffix.
