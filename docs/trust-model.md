...
Simplified Chinese prompt-risk rules cover action-first and 把/将 object-first exfiltration forms, including bounded modifiers before the protected object. Chinese and ASCII sentence punctuation bound those spans. For Markdown paragraphs, `remark-parse` and `mdast-util-to-string` also expose soft-wrapped rendered prose to the same rules, while blank paragraph boundaries remain separate. Privileged-role findings require an explicit direct or temporally qualified identity assignment followed by a clause boundary, conjunction, or aspect continuation; an impersonation verb with a privileged role or message noun; or an `以…身份` cue using the complete role noun. Assignment detection is independent of harmless imperative prefixes; the ICU-backed `Intl.Segmenter` identifies its containing `zh-Hans` sentence before conditional or interrogative cues are evaluated. That context filter applies only to direct identity assignments, never explicit impersonation or `以…身份`. Conditional or interrogative reader descriptions, compound role nouns, ordinary elevated-permission instructions, and prose about simulated system behavior remain outside that identity category.
