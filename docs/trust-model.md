...
The Codex capability-envelope validation primitive is available, but it neither establishes a native Codex transport nor grants capabilities by host name. The adapter creates a non-repeating challenge for every discovery. A trusted native integration must bind every envelope to that challenge and the expected session, snapshot, producer, validity window, maximum lifetime, and capability ceiling.
