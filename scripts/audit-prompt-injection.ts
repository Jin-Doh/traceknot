...
export const RULES: readonly Rule[] = [
  ...
  },
  {
    ...
    score: 7,
    pattern: /(?:(?:显示|打印|泄露|发送|上传|导出).{0,32}(?:秘密|令牌|凭据|私钥|API\s*密钥|系统提示|开发者消息|环境变量|\.env)|(?:把|将)[^。；，！？\n]{0,12}(?:秘密|令牌|凭据|私钥|API\s*密钥|系统提示|开发者消息|环境变量|\.env).{0,32}(?:显示|打印|泄露|发送|上传|导出))/i,
  },
  {
    id: "PI011",
  ...
  },
  {
    ...
    score: 6,
    pattern: /(?:(?<!如果)(?<!假如)(?<!倘若)(?<!若)(?<!当)(?:现在|从现在起)?(?:你|您)(?:现在|从现在起|将)?是(?:一(?:名|个|位))?(?:系统(?:管理员|开发者)?|开发者|管理员|超级用户|root)(?=$|[\s，。；！？!?、]|并(?:且)?|而|然后|随后|就|将|会|要|已(?:经)?|了|着|过|请)|(?:假装|扮演|伪装|冒充)(?:成|为)?(?:系统(?:管理员|开发者|身份|消息|指令|角色)|开发者|管理员|超级用户|root)|以(?:系统(?:管理员|开发者)?|开发者|管理员|超级用户|root)(?:的)?身份)/iu,
  },
];
