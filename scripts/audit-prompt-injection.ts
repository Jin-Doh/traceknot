...
export const RULES: readonly Rule[] = [
  ...
  },
  {
    ...
    score: 8,
    pattern: /(?:\b(?:curl|wget|nc|ncat|netcat|socat)\b[^\n]{0,240}(?:(?:@(?:[A-Za-z0-9._~\/-]+\/)?(?:\.env|(?:~\/)?\.ssh\/(?:id_[a-z0-9_-]+)|(?:credential|secret|token|password))|[$%]\{?(?:TOKEN|SECRET|PASSWORD|CREDENTIAL|API_KEY)\}?)[^\n]{0,240}(?:https?:\/\/|wss?:\/\/|(?:[a-z0-9-]+\.)+[a-z]{2,}\b)|(?:https?:\/\/|wss?:\/\/|(?:[a-z0-9-]+\.)+[a-z]{2,}\b)[^\n]{0,240}(?:@(?:[A-Za-z0-9._~\/-]+\/)?(?:\.env|(?:~\/)?\.ssh\/(?:id_[a-z0-9_-]+)|(?:credential|secret|token|password))|[$%]\{?(?:TOKEN|SECRET|PASSWORD|CREDENTIAL|API_KEY)\}?))|\b(?:pipe|cat|type|read)\b[^\n]{0,120}(?:\.env|(?:~\/)?\.ssh\/id_[a-z0-9_-]+|(?:credential|secret|token|password))[^\n]{0,80}\b(?:into|\|)\s*(?:nc|ncat|netcat|socat|curl|wget)\b)/i,
  },
  {
    id: "PI018",
