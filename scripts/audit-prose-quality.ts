...
function markdownBlockquotes(text: string): string[] {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const containerIndent = listMarker ? visualColumns(`${listMarker[1]}-${listMarker[2]}`) : 0;
...
function protectedValues(text: string): Map<string, { category: string; count: number }> {
  const categories: Array<[string, RegExp]> = [
    ...
    ["number", /\b(?:zero|one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|hundred|thousand|million|billion|trillion)\b(?=\s+(?:retries?|attempts?|items?|users?|deployments?|records?|files?|requests?|seconds?|minutes?|hours?|days?|weeks?|months?|years?|bytes?|bits?))|(?:한|두|세|네|다섯|여섯|일곱|여덟|아홉|열)(?=\s*(?:개|명|건|회|원|년|월|일|시간|분|초|대|권|장|마리|곳|배))/gi],
